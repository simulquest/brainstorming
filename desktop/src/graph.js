// src/graph.js - Éditeur graphique plein écran (zoom/pan + édition + branches)
import { elements, getActiveConversation } from './state.js';
import { setStatus } from './ui.js';
import { saveData } from './persistence.js';
import { renderMermaid, sanitizeMermaidCode } from './mermaid.js';
...
export const renderCanvas = async (code) => {
  ...
  if (!code) return;
  const visibleCode = sanitizeMermaidCode(filterBranches(code, hiddenBranchIds));
  if (!visibleCode.trim()) {
  ...
  try {
    ...
    if (typeof mermaid.parse === 'function') {
      await mermaid.parse(visibleCode);
    }
    const { svg } = await mermaid.render(`graph-full-${Date.now()}-${(graphRenderSeq = (graphRenderSeq + 1) % 100000)}`, visibleCode);
    elements.graphRender.innerHTML = svg;
  } catch (error) {
    console.error('Erreur de rendu plein écran :', error);
    const msg = String(error?.message || error || '').slice(0, 300).replace(/</g, '&lt;');
    elements.graphRender.innerHTML = `<p class="mermaid-error">Code Mermaid invalide (${msg}). Corrigez-le puis cliquez sur Appliquer.</p>`;
  }
  applyTransform();
};
...
export const applyGraphCode = async () => {
  const rawCode = elements.graphCodeEditor?.value?.trim() || '';
  if (!rawCode) {
    setStatus('Le code Mermaid est vide.', true);
    return;
  }
  const code = sanitizeMermaidCode(rawCode);
  if (elements.graphCodeEditor && code !== rawCode) elements.graphCodeEditor.value = code;
  const active = getActiveConversation();
